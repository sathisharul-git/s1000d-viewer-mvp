...
public class RenderFacade {
    ...
    public RenderedDm render(DataModuleDescriptor descriptor, ApplicabilityResult applicabilityResult) {
        if (renderProperties.isPublishedPreferred()) {
            return publishedRenderService.render(descriptor.dmId(), descriptor, applicabilityResult)
                .orElseGet(() -> renderQuickOrFail(descriptor, applicabilityResult));
        }

        if (renderProperties.isQuickPreviewEnabled()) {
            return quickRenderService.render(descriptor.dmId(), descriptor, applicabilityResult);
        }
    ...
    private RenderedDm renderQuickOrFail(DataModuleDescriptor descriptor, ApplicabilityResult applicabilityResult) {
        if (!renderProperties.isQuickPreviewEnabled()) {
